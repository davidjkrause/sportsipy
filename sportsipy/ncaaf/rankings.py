...
class CFPRankings:
    ...
    def _pull_rankings_page(self, year):
        ...
        """
        try:
            url_data = pq(url=CFP_RANKINGS_URL % year)
            return pq(utils._remove_html_comment_tags(url_data))
        except HTTPError:
            return None
...
class CoachesRankings:
    ...
    def _pull_rankings_page(self, year):
        ...
        """
        try:
            url_data = pq(url=COACHES_RANKINGS_URL % year)
            return pq(utils._remove_html_comment_tags(url_data))
        except HTTPError:
            return None
    ...
    def _find_rankings(self, year):
        ...
        if not year:
            ...
            # previous year's stats. If it exists, use the previous year
            # instead.
            if not utils._url_exists(COACHES_RANKINGS_URL % year) and \
               utils._url_exists(COACHES_RANKINGS_URL % str(int(year) - 1)):
                year = str(int(year) - 1)
        page = self._pull_rankings_page(year)
        if not page:
            output = ("Can't pull rankings page. Ensure the following URL "
                      "exists: %s" % COACHES_RANKINGS_URL)
            raise ValueError(output)
